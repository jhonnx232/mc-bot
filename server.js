...
io.on("connection", (socket) => {
  ...
  socket.on("start-bot", (data) => {
    console.log(
      `🎮 Tentando conectar bot: ${data.username} em ${data.host}:${data.port} (v${data.version})`
    );
  ...
  socket.on("disconnect", () => {
    console.log(
      "🛑 App desconectado. O bot continuará rodando se configurado."
    );
  });
});
...
http.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `DICA: No Cordova, use o IP da sua rede (ex: 192.168.x.x) para conectar.`
  );
});
